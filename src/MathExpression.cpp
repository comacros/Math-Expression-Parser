...
protected:
    ...
    bool GetSubExpressionLength(const char* lpcszExpr, size_t& nExprSubLength)    // leading '(' and ending ')' included
    {
        size_t nExprLength = strlen(lpcszExpr);
...
{
    MathExpression me("x ^ 2 / (x + 1)");
    map<string, vector<double> > symbols;
    vector<double> x(10000000);
    for(size_t i = 0; i < x.size(); i++)
        x[i] = i / 5000000.0;
    symbols["x"] = x;
    vector<double> results;
    double tick1 = GetTickCount();
    bool bRet = me.Evaluate(results, symbols);
    double tick2 = GetTickCount();
    if(!bRet)
        printf("error");
    printf("Time: %g ms\n", tick2 - tick1);
    for(size_t i = 0; i < x.size(); i += 1000000)
        printf("f(%g) = %g\n", x[i], results[i]);
